...
import { loadBudget, spendSince, startOfLocalDay, evaluateBudget } from '../budget';
...
// The soft cap lives here rather than in the agent's context: it is a nudge for the
// person, and injecting it into the conversation would spend tokens to say "you are
// spending tokens". A hard cap is enforced by the guard hook, not announced here.
{
  const cfg = loadBudget();
  const daily = spendSince(openDbReadOnly(), startOfLocalDay(Date.now()));
  const breach = evaluateBudget(cfg, { exact: 0, estimated: 0, unmeasuredCalls: 0, calls: 0 }, daily);
  if (breach.level !== 'ok') {
    parts.push(`${breach.level === 'hard' ? '!!' : '!'} budget ${usd(breach.spend)}/${usd(breach.cap)}`);
  }
}
