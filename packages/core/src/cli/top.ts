...
import { getYieldSummary } from '../yield';
import { ago, compact, mmss, usd } from '../util/format';

const args = process.argv.slice(2);
const sinceArg = args.find((a) => a.startsWith('--since='))?.split('=')[1];
const sinceMin = Number(sinceArg ?? 30);
// NaN would make every window comparison false and silently show nothing at all,
// which reads as "no activity" rather than "you typed the flag wrong".
if (!Number.isFinite(sinceMin) || sinceMin <= 0) {
  console.error(`--since must be a positive number of minutes, got "${sinceArg}"`);
  process.exit(2);
}
...
function frame(): void {
  ...
  // Yield beside the live view: "is this producing anything?" is the incident feed's
  // question one level up. Unclassified sessions are named, never folded into the ratio.
  const y = hasYield() ? getYieldSummary(db, now - 7 * 24 * 3600_000) : null;
  if (y && y.abandonedShare !== null) {
    out.push(
      `7d yield · ${usd(y.abandoned.cost)} of ${usd(y.committed.cost + y.abandoned.cost)} ` +
        `(${Math.round(y.abandonedShare * 100)}%) produced no commit` +
        (y.unclear.sessions ? ` · ${y.unclear.sessions} unclassified` : ''),
    );
  }
...
/** A store from before migration 31 has no yield table; asking beats throwing. */
function hasYield(): boolean {
  const r = db
