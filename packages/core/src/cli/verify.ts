...
if (existsSync(root)) {
  ...
  for (const f of walkTranscripts(root)) {
    for (const line of readFileSync(f, 'utf8').split('\n')) {
      ...
      // `null` and scalars parse fine; only an object can carry the fields below.
...
// ── Cursor: the editor state store is the source of truth for its exact rows.
// Bubbles are rewritten in place as a conversation grows but their tokenCount is
// final once set, so every stored row must still be re-derivable from it. ──
const curTruth = new Map<string, number>();
const curPath = paths.cursorStateDb();
let curSourceReadable = false;
if (existsSync(curPath)) {
  try {
    const src = new Database(curPath, { readonly: true, fileMustExist: true });
    try {
      for (const b of src
        .prepare(
          `SELECT key,
        ...
        .all() as { key: string; inTok: number | null; outTok: number | null }[]) {
        const parts = b.key.split(':');
        if (parts.length < 3) continue;
        const total = (b.inTok ?? 0) + (b.outTok ?? 0);
        if (total <= 0) continue;
        curTruth.set(`cursor:bubble:${parts[1]}:${parts[2]}`, total);
      }
      curSourceReadable = true;
    } finally {
      src.close();
    }
  } catch {
    /* store held exclusively or unreadable: reconcile nothing rather than fail all */
  }
}

let curReconciled = 0;
let curTokenMismatch = 0;
let curNotInStore = 0;
{
  const rows = db
    .prepare(
      "SELECT event_key, total_tokens FROM usage_events WHERE tool='cursor' AND source='live' AND confidence='exact'",
    )
    .all() as { event_key: string; total_tokens: number | null }[];
  for (const r of rows) {
    const t = curTruth.get(r.event_key);
    if (t === undefined) {
      // Only a real orphan when the source was actually readable; an unreadable
      // store must not be reported as "stored but not in source".
      if (curSourceReadable) curNotInStore++;
      continue;
    }
    curReconciled++;
    if (r.total_tokens !== t) curTokenMismatch++;
  }
}
...
console.log(`  [cursor] bubbles with tokens     ${curTruth.size}`);
console.log(`  [cursor] reconciled per-message  ${curReconciled}`);
console.log(`  [cursor] token mismatches        ${curTokenMismatch}`);
console.log(`  [cursor] stored but not in store ${curNotInStore}  (expect 0)`);
