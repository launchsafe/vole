...
import { boundNote, drainInbox, insertAnomalies, insertEvents, openDb, pruneRetiredCollectorRuns, purgeRetiredRules, recordCollectorRun, recordScan, repriceUnpriced } from '../db';
...
function runOnce(): void {
  ...
  if (totalInserted > 0 || rateLimits.length > 0 || epochChanged) {
    ...
    // Enforce the registry invariant on every detect pass, not only on an epoch
    // change: rows left by a build whose rules have since been retired outlive the
    // epoch that retired them, so an epoch-gated purge would never reach them.
    // Nothing filters `rule` at read time, so the store is where it has to hold.
    retiredPurged = purgeRetiredRules(db, RULE_IDS);
    // Yield runs in the same insert-gated block: it only has new work when new rows
    // arrived, and a settled verdict is never recomputed, so the steady-state cost is
    // one query rather than a git process per session.
    yieldClassified = computeYield(db, now);
    if (epochChanged) {
  ...
  console.log(
    `[${new Date().toISOString()}] parsed ${fmt(totalFound)} events, ` +
      ...
      `${fmt(newAnomalies.length)} new${escalatedAnomalies.length ? `, ${fmt(escalatedAnomalies.length)} escalated` : ''}${retiredPurged ? `, ${fmt(retiredPurged)} from retired rules purged` : ''}${yieldClassified ? `, ${fmt(yieldClassified)} sessions yield-checked` : ''} (${ms}ms)`,
  );
