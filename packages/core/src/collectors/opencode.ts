...
export function collectOpencode(_db: DB): CollectorResult {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      const input = count(t.input);
      const output = count(t.output);
      const reasoning = count(t.reasoning);
      const read = count(t.cache?.read);
      const write = count(t.cache?.write);
      ...
      events.push({
        ...
        total_tokens: sumCounts(input, output, reasoning, read, write),
