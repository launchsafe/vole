...
import { setState, type DB } from '../db';
import { parseLine, count, sumCounts } from '../util/jsonl';
...
export function collectGrok(db: DB): CollectorResult {
  ...
  try {
    // Full re-read each pass (cheap, 18ms) so cross-chunk exec_done attribution
    // stays correct.
    lines = readFileSync(logPath, 'utf8').split('\n');
    st = statSync(logPath);
  } catch (err) {
    return { tool: 'grok', events, filesScanned: 0, notes: [`Could not read ${logPath}: ${(err as Error).message}`], sourceState: 'error' };
  }
  ...
  for (const raw of lines) {
    ...
    const c = e.ctx;
    const prompt = count(c.prompt_tokens);
    const cached = count(c.cached_prompt_tokens);
    const output = count(c.completion_tokens);
    ...
    const ev: UsageEvent = {
      ...
      total_tokens: sumCounts(prompt, output),
  ...
  // The read offset for the unified log: a record of where this source stopped.
  // The log is still re-READ in full each pass (exec_done attribution crosses
  // chunk boundaries); the offset is the record, not the gate.
  setState(db, logPath, 'grok', st.size, Math.trunc(st.mtimeMs));
