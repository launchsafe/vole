...
import { getYieldSummary, type YieldSummary } from './yield';
 ...
 */
/**
 * Ceiling on zero-filled buckets. A continuous axis is a rendering aid; past a few
 * thousand points nothing is legible anyway, so an absurd span (an ancient or
 * future `ts` from a third-party log) drops the fill rather than hanging the caller.
 */
...
export function getTimeseries(db: DB, range: Range, includeSeed: boolean, now = Date.now()): TimePoint[] {
  ...
  const first = range === 'all' ? Math.min(...buckets) : Math.floor(from / bucketMs) * bucketMs;
  // Never fill past now. `ts` comes from someone else's log: one broken clock writing
  // year 3999 would otherwise zero-fill ~17M hourly buckets, one allocation each.
  // Rows beyond now stay in the map — they are reported, just not filled up to.
  const last = nowBucket;
  if ((last - first) / bucketMs <= MAX_FILL_BUCKETS) {
...
export function getLiveSessions(
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      // Clamped to now + ttl: `e.ts` is a third-party clock, and one skewed into the
      // future rendered a "cache expires in" of roughly two thousand years. A prompt
      // cache cannot outlive its TTL, so that is the only honest ceiling.
      cache_expires_at: ttl && e ? Math.min(e.ts + ttl, now + ttl) : null,
      // Price the re-warm at the cache tier the session is ACTUALLY on. cacheTtlMs
      // above already distinguishes them (a 1h TTL means the turn wrote to the 1h
      // cache), but this always used the 5m multiplier — and 1h writes cost 60% more
      // per token, so a 1h session's re-warm read as 62.5% of its real price.
      rewarm_cost: rate && ctx > 0
        ? (ctx * rate.input
           * (ttl === 3_600_000 ? CACHE_MULTIPLIERS.write1h : CACHE_MULTIPLIERS.write5m)) / 1e6
        : null,
      incidents: {
        count: sev.length,
...
/** `anthropic/claude-opus-5` -> `claude-opus-5`; every other provider prefix is left
 *  alone so a proxy's rows stay unpriced rather than priced at first-party rates. */
function firstPartyModel(model: string | null): string | null {
...
export interface RewarmSummary {
  ...
  tokens: number;
  cost: number | null;
  /** Models with re-warm tokens but no resolvable rate (a proxied `provider/model`
   *  id). `cost` is the sum over the rest, so a non-zero count means it is a floor,
   *  not a total — the caller must say so rather than print a partial sum as whole. */
...
export function getCacheRewarm(db: DB, range: Range, includeSeed: boolean): RewarmSummary {
  ...
  const byModel: RewarmRow[] = rows.map((r) => ({
    ...
    gaps: r.gaps,
    tokens: r.w5 + r.w1,
    // OpenCode reports `provider/model` (github-copilot/claude-opus-4.6), which no
    // rate resolves. Only the first-party `anthropic/` prefix is stripped — the same
    // rule contextWindow() uses, and for the same reason: a proxy provider may bill
    // differently, so pricing its rows at list price would invent a number.
    cost: computeCost(firstPartyModel(r.model), { cache_write_5m_tokens: r.w5, cache_write_1h_tokens: r.w1 }),
  }));
  const costs = byModel.map((r) => r.cost).filter((c): c is number => c !== null);
  // A partial sum must not be presented as a whole one: rows whose model has no
  // resolvable rate used to drop out of the total silently, understating it.
  const unpriced = byModel.filter((r) => r.cost === null && r.tokens > 0).length;
  return {
...
export interface ModelComparisonRow {
  tool: Tool;
  model: string | null;
  /** Worst tier present in the row, so a mixed model never reads as fully exact. */
  confidence: Confidence;
  calls: number;
  tokens: number;
  cost: number | null;
  /** Null where the model is unpriced — an efficiency figure needs a cost to exist. */
...
/**
 * Per-model efficiency, for comparing what you actually run against each other.
 *
 * One row per (tool, model, tier). Splitting by tier rather than collapsing to the
 * worst one is what keeps a row internally consistent: a tool whose history spans
 * tiers — Cursor, which recorded tokens until April 2026 and not since — would
 * otherwise report its real token total under a "no tokens" label.
 *
 * activity_only calls count toward `calls` and `errorRate` but contribute no tokens or
 * cost, so a model Vole cannot measure shows a real call count beside an empty cost
 * rather than a flatteringly cheap one.
 */
export function getModelComparison(db: DB, range: Range, includeSeed: boolean): ModelComparisonRow[] {
  const from = rangeStart(range);
  const seedClause = includeSeed ? '' : " AND source = 'live'";
  const rows = db
    .prepare(
    ...
    .all(from) as {
    tool: Tool; model: string | null; calls: number; tokens: number; cost: number | null;
    uncosted: number; cache_read: number; input: number; errors: number; worst: string;
  }[];

  return rows.map((r) => {
    const denom = r.cache_read + r.input;
    return {
      tool: r.tool,
      model: r.model,
      // Grouped BY confidence rather than collapsed to the worst tier in the group.
      // Collapsing produced rows that contradicted themselves — Cursor reported
      // "65.0M tokens" under a "no tokens" label, because its exact and activity_only
      // rows merged. A tool that spans tiers now shows one row per tier.
      confidence: r.worst as Confidence,
      calls: r.calls,
      tokens: r.tokens,
      cost: r.uncosted === r.calls ? null : r.cost,
      costPerCall: r.cost !== null && r.uncosted === 0 && r.calls > 0 ? r.cost / r.calls : null,
      tokensPerCall: r.calls > 0 ? r.tokens / r.calls : 0,
      cacheHitRatio: denom > 0 ? r.cache_read / denom : null,
      errorRate: r.calls > 0 ? (r.errors ?? 0) / r.calls : 0,
    };
  });
}
...
export interface Digest {
  ...
  /** Did the spend produce commits? Null when the store predates yield tracking. */
  yield: YieldSummary | null;
}
...
export function getDigest(db: DB, range: Range, includeSeed: boolean, now = Date.now()): Digest {
  ...
  // getTimeseries buckets HOURLY for the 24h range, so taking its max there reported
  // the busiest HOUR under a label that reads (and formats) as a date. Roll the points
  // up to calendar days first so "busiest day" is a day at every range.
  const DAY_MS = 24 * 3600_000;
  const byDay = new Map<number, number>();
  for (const p of getTimeseries(db, range, includeSeed, now)) {
    const day = Math.floor(p.bucket / DAY_MS) * DAY_MS;
    const tokens = (Object.keys(ZERO_BY_TOOL) as Tool[]).reduce((s, t) => s + (p[t] ?? 0), 0);
  ...
  return {
    ...
    // Null on a store written before yield tracking existed, so a caller can tell
    // "nothing was classified" from "this store cannot answer the question".
    yield: hasYieldTable(db) ? getYieldSummary(db, from) : null,
  };
}

/** A store from before migration 31 has no such table; asking is cheaper than failing. */
function hasYieldTable(db: DB): boolean {
  const r = db
    .prepare("SELECT 1 AS ok FROM sqlite_master WHERE type = 'table' AND name = 'session_yield'")
