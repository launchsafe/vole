...
export function detectErrorStorms(events: UsageEvent[], now: number): Anomaly[] {
  const out: Anomaly[] = [];
  // Every real call counts. The old filter dropped activity_only rows on the premise
  // that they "carry no error signal at all" — false for grok, which records a failed
  // call as exactly that: an activity_only row with is_error = 1 (see grok.ts). On a
  // live store that hid 969 of grok's 1,758 calls, so this rule could never fire for
  // it. is_error is the signal; confidence only says whether TOKENS were measured.
  // Successful activity_only rows still belong in the denominator, or the ratio would
  // be computed against errors alone and trip on any five failures.
  const usable = events;
