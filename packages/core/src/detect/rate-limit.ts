...
export function detectRateLimitPressure(
  ...
  // One anomaly per (tool, session, hour) — reporting the WORST reading in the
  // bucket, not the first one seen. Keeping the first meant a bucket that opened at
  // 82% and later peaked at 97% was filed as `warn` and never escalated to
  // `critical`, understating exactly the case the rule exists to catch. Observations
  // arrive in no guaranteed order, so pick the max before emitting.
  const worst = new Map<string, RateLimitObservation & { bucket: number }>();
  for (const o of observations) {
    ...
    if (!prev || o.used_percent > prev.used_percent) worst.set(key, { ...o, bucket });
  }

  for (const [key, o] of worst) {
