...
import { detectBySource } from './index';
import type { RateLimitObservation, UsageEvent } from '../types';
...
test('quota-only sources produce alerts without stored usage', () => {
  const observation: RateLimitObservation = {
    tool: 'codex', session_id: 'quota-only', ts: T0, used_percent: 97, window_minutes: 300,
  };
  const expected = detectRateLimitPressure([observation], T0).map((a) => ({
    ...a, anomaly_key: `live:${a.anomaly_key}`,
  }));
  assert.equal(expected.length, 1);
  assert.deepEqual(detectBySource([], { live: [observation] }, T0), expected);
  assert.deepEqual(detectBySource([ev({ source: 'seed' })], { live: [observation] }, T0), expected);
  assert.deepEqual(detectBySource([ev()], { live: [observation] }, T0), expected);
  assert.deepEqual(detectBySource([], { live: [] }, T0), []);
  assert.deepEqual(detectBySource([], { live: undefined }, T0), []);
  const both = detectBySource([], { live: [observation], seed: [observation] }, T0);
  assert.deepEqual(both.map((a) => a.source), ['live', 'seed']);
  assert.equal(new Set(both.map((a) => a.anomaly_key)).size, 2);
});
