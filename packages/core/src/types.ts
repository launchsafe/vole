...
/**
 * How much we actually know about a row's numbers.
 *
 * - `exact`         Token counts come verbatim from the tool's own logs. `total_tokens`
 *                   is a plain sum (or, for Codex, a delta) of real fields.
 * - `estimated`     The tool records no usable count, but a DETERMINISTIC, documented
 *                   method derives one from something it does record. The method is
 *                   named in `estimation_method` and its inputs are stored, so
 *                   `pnpm verify` re-runs it and fails on any mismatch. An estimate
 *                   that cannot be re-derived exactly from its stated method is a bug,
 *                   not an estimate — that is the whole difference between this tier
 *                   and a guess, and why the earlier no-estimates policy was safe to
 *                   relax. These rows never merge into an `exact` total unlabelled.
 * - `activity_only` The tool records that a call happened and NOTHING that could
 *                   support even a falsifiable estimate. These rows carry NULL tokens
 *                   and are excluded from every token and cost aggregate, so they can
 *                   never inflate a total.
 *
 * The ordering matters: prefer `exact` wherever the data exists, fall back to
 * `estimated` only where a real method exists, and leave a row `activity_only`
 * rather than invent an unfalsifiable number for it.
 */
export type Confidence = 'exact' | 'estimated' | 'activity_only';

/** Every tier, worst-known-last — the canonical order for filters and UI legends. */
export const CONFIDENCE_TIERS: readonly Confidence[] = ['exact', 'estimated', 'activity_only'];

/**
 * How an `estimated` row's numbers were derived. One id per documented method, so
 * `pnpm verify` can look up the re-derivation for a stored row. NULL on every row
 * whose confidence is not `estimated`.
 */
export type EstimationMethod = 'content_length_heuristic' | 'reply_text_ratio';
...
export interface UsageEvent {
  ...
  /** Only set when confidence is `estimated`; names the method verify re-runs. */
