...
export const paths = {
  ...
  /** Cursor AI attribution DB: models, sessions and code-hash attribution, no token counts. */
  cursorTrackingDb: () =>
    process.env.VOLE_CURSOR_DB ?? join(home(), '.cursor', 'ai-tracking', 'ai-code-tracking.db'),

  /**
   * Cursor's editor state store — where the token counts actually live.
   *
   * `cursorDiskKV` holds one `bubbleId:<composerId>:<bubbleId>` row per message, and
   * those rows carry a real `tokenCount: { inputTokens, outputTokens }`. The tracking
   * DB above has no token columns at all, which is why Cursor was long recorded as
   * activity-only; the two stores use disjoint id spaces and do not join.
   */
  cursorStateDb: () =>
    process.env.VOLE_CURSOR_STATE_DB ??
