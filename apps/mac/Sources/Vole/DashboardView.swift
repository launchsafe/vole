...
enum DashSection: String, CaseIterable, Identifiable {
    ...
    var icon: String {
        ...
        case .findings:  return "lightbulb"
    ...
    var requires: [String]? {
        ...
        case .findings: return ["optimize_findings"]
        }
    }
    ...
    var group: SidebarGroup {
        ...
        case .incidents, .findings: return .risk
    ...
    func badge(counts: SectionCounts) -> Int {
        ...
        case .findings: return counts.findings
        default: return 0
        }
    }
}
...
struct SectionCounts {
    ...
    static func compute(_ store: Store) -> SectionCounts {        var c = SectionCounts()
        ...
        c.findings = store.findings.filter { $0.revertedAt == nil }.count
        return c
    }
}
...
private struct NavIcon: View {
    ...
    @ViewBuilder private var symbol: some View {
        ...
        case .findings:  img.symbolEffect(.bounce, options: .nonRepeating, value: pulse)
...
struct DashboardView: View {
    ...
    private var dashboardPane: some View {
        Form {
            ...
            Section {
                ...
                metricRow("Equivalent Cost", Fmt.summaryCost(s), "dollarsign", .green, prominent: true)
                    .help(Fmt.summaryCostHelp(s))
                    .accessibilityHint(Fmt.summaryCostHelp(s))
            ...
            Section("Activity") {
                ...
                // Yield sits with the activity metrics rather than in its own panel:
                // "did this produce anything" belongs beside calls and errors, and a
                // separate card would overstate a figure that is often mostly unknown.
                if let y = store.yieldSummary, let share = y.abandonedShare {
                    metricRow("No commit", Fmt.pct(share), "arrow.triangle.branch",
                              share >= 0.5 ? Pal.warning : .secondary)
                }
            }
    ...
    /// `vole optimize`'s findings, read-only.
    ///
    /// Deliberately does not offer an Apply button. Applying writes to a config file on
    /// disk and schedules a verification three days out; a one-click version of that,
    /// with no way to see what it wrote, is exactly the kind of thing a monitoring tool
    /// should not do quietly. The command is named instead.
        ...
        Group {
            if store.findings.isEmpty {
                ContentUnavailableView(
                    ...
                    description: Text("Run `pnpm optimize` to scan for waste. Findings appear here."))
            } else {
                List {
                    ForEach(store.findings) { f in
                        VStack(alignment: .leading, spacing: 6) {
                            HStack(alignment: .firstTextBaseline, spacing: 8) {
                                ...
                                Spacer()
                                if let p = f.predicted, p > 0 {
                                    Text("up to \(Fmt.money(p))")
                    ...
                    Section {
                        Text("Applying a fix, and re-checking it three days later, is `pnpm optimize --apply`. Anything that did not help is reverted automatically.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                }
