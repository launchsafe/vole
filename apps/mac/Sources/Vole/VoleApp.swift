    ...
    init() {
        ...
        if CommandLine.arguments.contains("--dump") {
            Task.detached {
                ...
                print("24h  calls=\(s.calls)  tokens=\(s.tokens)  cost=\(Fmt.summaryCost(s))  sessions=\(s.sessions)  unpricedCalls=\(s.unpricedCalls)")
...
struct MenuBarLabel: View {
    ...
    var body: some View {
        ...
        let help = state.label + (state.showsFigure && menubar == "cost"
                                  ? ". " + Fmt.summaryCostHelp(store.summary) : "")
        Group {
