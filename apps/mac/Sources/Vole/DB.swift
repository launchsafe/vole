...
/// Did a session's spend produce a commit?
struct YieldSummary {
    let committedSessions: Int
    let committedCost: Double
    let abandonedSessions: Int
    let abandonedCost: Double
    let unclearSessions: Int
    /// Share of CLASSIFIED cost with no commit behind it. Unknowns stay out of the
    /// denominator: a store full of non-git projects would otherwise report a
    /// reassuringly low number that means nothing.
    var abandonedShare: Double? {
        let classified = committedCost + abandonedCost
        return classified > 0 ? abandonedCost / classified : nil
    }
}

/// One `vole optimize` finding, as the CLI stored it.
struct FindingRow: Identifiable {
    ...
    let outcome: String?
    let revertedAt: Int?
    /// What this row is doing right now, for a badge. Nil when there is nothing to say.
    var state: String? {
        if revertedAt != nil { return "reverted" }
        if let o = outcome { return o.replacingOccurrences(of: "_", with: " ") }
        if appliedAt != nil { return "verifying" }
        return mechanical ? "can apply" : nil
    }
}
...
actor DB {
    ...
    static let knownSchemaVersion = 32
    ...
    private nonisolated static func open(_ path: String) -> OpaquePointer? {
        ...
        // sqlite3_open_v2 never reads the file — it only allocates a connection, so
        // success here proves nothing. A connection is accepted only once it has
        // actually read a page, which is what separates a real store from 8KB of
        // noise, and what tells a usable READONLY handle from one that opened but
        // cannot serve this database.
        func attempt(_ flags: Int32) -> OpaquePointer? {
            ...
            if readable(handle) { return handle }
            sqlite3_close_v2(handle)
            return nil
        }
        // WAL databases sometimes refuse a pure READONLY connection — it cannot create
        // the -shm index the WAL reader needs. That refusal only surfaces on the first
        // read, never at open, so proving the read above is what makes this fallback
        // real: before, a WAL store kept the unusable READONLY handle and the app
        // reported a healthy database while showing no data at all.
        return attempt(SQLITE_OPEN_READONLY) ?? attempt(SQLITE_OPEN_READWRITE)
    }

    /// Runs the cheapest query that forces a real page read, to prove the file is a database.
    private nonisolated static func readable(_ h: OpaquePointer) -> Bool {
        var stmt: OpaquePointer?
        guard sqlite3_prepare_v2(h, "SELECT count(*) FROM sqlite_master", -1, &stmt, nil) == SQLITE_OK
        else { return false }
...
// MARK: queries
    ...
    func summary(_ r: DateRange) -> Summary {
        ...
        run("""
            SELECT COUNT(*),
                   ...
                   COALESCE(SUM(CASE WHEN cost_usd IS NULL AND \(tf) THEN 1 ELSE 0 END), 0)
    ...
    /// Mirrors MAX_FILL_BUCKETS in queries.ts — past a few thousand points a chart
    /// is unreadable, so an absurd span drops the fill instead of hanging the UI.
    private static let maxFillBuckets = 10_000
    ...
    func timeseries(_ r: DateRange) -> [TimePoint] {
        ...
        // Never fill past now, and never fill an absurd span: `ts` comes from another
        // tool's log, and one broken clock writing year 3999 zero-filled ~17M hourly
        // buckets here — a ~55s freeze on every refresh. Rows beyond now stay in the
        // map, so they are still charted; they just no longer stretch the axis.
        let last = nowBucket
        if last > first, (last - first) / b <= Self.maxFillBuckets {
            var k = first
            while k <= last {
    ...
    /// Yield over a range. Nil on a store written before yield tracking existed, so the
    /// UI can tell "nothing classified yet" from "this store cannot answer the question".
    func yieldSummary(_ r: DateRange) -> YieldSummary? {
        guard tableNames().contains("session_yield") else { return nil }
        let from = r.startMs()
        var committedN = 0, committedCost = 0.0
        var abandonedN = 0, abandonedCost = 0.0
        var unclearN = 0
        run("""
            SELECT y.status, COUNT(*), COALESCE(SUM(c.cost), 0)
             ...
             GROUP BY y.status
            """, [from]) { row in
            let status = colText(row, 0) ?? ""
            let n = colInt(row, 1)
            let cost = colDblOpt(row, 2) ?? 0
            switch status {
            case "committed": committedN = n; committedCost = cost
            case "abandoned": abandonedN = n; abandonedCost = cost
            default: unclearN = n
            }
        }
        return YieldSummary(
            committedSessions: committedN, committedCost: committedCost,
            abandonedSessions: abandonedN, abandonedCost: abandonedCost,
            unclearSessions: unclearN)
    }

    /// Findings from `pnpm optimize`, most valuable first. Empty on an older store.
    func findings() -> [FindingRow] {
        guard tableNames().contains("optimize_findings") else { return [] }
        var out: [FindingRow] = []
        run("""
            SELECT id, kind, title, detail, fix, mechanical, predicted_usd, baseline_usd,
                   applied_at, outcome, reverted_at
              FROM optimize_findings
             ORDER BY COALESCE(predicted_usd, 0) DESC, detected_at DESC
            """) { row in
            out.append(FindingRow(
                id: colInt(row, 0), kind: colText(row, 1) ?? "", title: colText(row, 2) ?? "",
                detail: colText(row, 3) ?? "", fix: colText(row, 4),
                mechanical: colInt(row, 5) == 1, predicted: colDblOpt(row, 6),
                baseline: colDblOpt(row, 7), appliedAt: colIntOpt(row, 8),
                outcome: colText(row, 9), revertedAt: colIntOpt(row, 10)))
        }
        return out
    }
}
